...
class DataDistribution(object):
    ...
    def sample(self, N):
        # 正态(高斯)分布，self.mu,均值 self.sigma,标准差 N个点
...
class GeneratorDistribution(object):
    ...
    def sample(self, N):
        # np.linspace创建等差数列-self.range~~~~self.range 创建N个数，组成等差数列
        # np.random.random返回N个随机的浮点数，在半开区间 [0.0, 1.0)。
...
class GAN(object):
    ...
    def train(self):
        with tf.Session() as session:
            ...
            for step in range(num_pretrain_steps):
                ...
                labels = norm.pdf(d, loc=self.data.mu, scale=self.data.sigma)  # 输入d，返回概率密度函数
            ...
            for step in range(self.num_steps):
                ...
                x = self.data.sample(self.batch_size)  # 正太分布
                z = self.gen.sample(self.batch_size)  # 随机分布
    ...
    def _samples(self, session, num_points=10000, num_bins=100):
        ...
        # histogram会return hist(array在各直方图中值的个数)和bin eages （每个直方图的边界）

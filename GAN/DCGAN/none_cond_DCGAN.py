...
from tensorflow.contrib.layers.python.layers import batch_norm as Tbatch_norm
    ...
    with tf.variable_scope(name) as scope:
        ...
        if is_train:
            ...
            with tf.control_dependencies([assign_mean, assign_variance]):
                # return Tbatch_norm(value, decay=0.9, epsilon=1e-5, scale=True,
...
def generator(z, is_train=True, name='generator'):
    with tf.name_scope(name):
        ...
        h1 = fully_connected(z, GF * 8 * s16 * s16, 'g_fc1')
        h1 = tf.reshape(h1, [-1, s16, s16, GF * 8], name='reshap')
...
def train():
    ...
    data_dir = CURRENT_DIR + '/img_align_celeba_tfrecords/'
